...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule){
        ...
        $schedule->call(function (){
            ...
            /// ---- MR AHMED Request Recharge report
            $data = RequestRechargeWallet::with(['staff','from_wallet.walletowner','to_wallet.walletowner','action_staff'])
                ->whereRaw("DATE(created_at) = ?",[$yesterday])
                ->get();

            Mail::to(Staff::find(16))
                ->send(new \App\Mail\RequestTransferEmail($data,$yesterday));
            /// ---- MR AHMED Request Recharge report
