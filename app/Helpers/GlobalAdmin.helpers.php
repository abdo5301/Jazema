...
function paymentDailyReportByEmail($date){
    ...
    $supervisor_wallets = \App\Models\Wallet::where('type','payment')
        ...
        ->whereIn('walletowner_id',array_column(\App\Models\Staff::whereIn('permission_group_id',explode(',',setting('sales_supervisor_group_id')))->get(['id'])->toArray(),'id'))
    ...
    $data['momkn_wallet']       = amount((string) \App\Libs\Payments\Adapters\Momkn::balance()->data->balance,true);
...
function adminDefineUserWithName($model,$id,$lang){
    switch($model){
        ...
        case 'App\Models\Merchant':
            $content = \App\Models\Merchant::where('id','=',$id)->first()->{'name_'.$lang};
