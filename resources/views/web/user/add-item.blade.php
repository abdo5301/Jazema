                                        ...
                                        <div id="option1" class="group task_content" style="">
                                            ...
                                            <div class="inp_group {!! formError($errors,'item_category_id',true) !!}">
                                                ...
                                                <select class="form-control" id="item_category_id"
                                                        ...
                                                        onchange='get_item_attribute(),get_merchant_template_option();'>
                                            ...
                                            <div class="inp_group {!! formError($errors,'item_type_id',true) !!}">
                                                ...
                                                <select class="form-control" name="item_type_id" id="item_type_id"
                                                        onchange='get_item_attribute(),get_item_type();'>
                                            ...
                                            <div class="inp_group">
                                               ...
                                               <div id="add_item_map" class="form-control"></div>
                                                    ...
                                                    <div id="new_option" style="display: none;">
                                                        ...
                                                        <div class="inp_group">
                                                            ...
                                                            <div class="form-control">
                                                                <label class="radio-inline"><input class="option_is_required" type="radio" name="option_is_required" value="yes" checked="checked">{{__('yes')}}</label>
                                                                <label class="radio-inline"><input  class="option_is_required" type="radio" name="option_is_required" value="no">{{__('no')}}</label>
                                                            </div>
                                                            </div>
...
@section('footer')
    ...
    <script>
        ...
        function initMap_add_item() {
            var myLatLng = {lat: 26.820553, lng: 30.802498}; // lat & lng of Egypt by default
            map5 = new google.maps.Map(document.getElementById('add_item_map'), {
                center: myLatLng,
                zoom: 8
            });

            google.maps.event.addListener(map5, 'click', function(event) {
                placeMarker_addItem(event.latLng);
        ...
        function placeMarker_addItem(location) {
            deleteOverlays_addItem();
            add_item_marker = new google.maps.Marker({
                map: map5,
                draggable: true,
                animation: google.maps.Animation.DROP,
                position: location
            });
            add_item_marker.addListener('click', toggleBounce_addItem);

        }

        function deleteOverlays_addItem() {
            if (add_item_marker) {
                add_item_marker.setMap(null);
            }
        }

        function toggleBounce_addItem() {
            if (add_item_marker.getAnimation() !== null) {
                add_item_marker.setAnimation(null);
            } else {
                add_item_marker.setAnimation(google.maps.Animation.BOUNCE);
            }
    ...
    <script>
        ...
        $('#itemForm').submit(function (e) {
            e.preventDefault();
            $.post('{{route('web.user.store-items')}}', $('#itemForm').find(":input").serialize(), function (out) {
                ...
                if (out.status == false) {
                    ...
                    $('#itemForm')[0].reset();
                    $('#item_type_div').css('display','none');
                    $("#image_list > *:not('#temp_id')").remove();
                    $('#temp_id').val('{{md5(uniqid() . time() . rand() . rand(1, 999999))}}');
                    $('#attribute_div_list_item').empty();
                    $("#option_li_list > *:not('#option_li_temp')").remove();
                    publicLength = $('#option_li_list li').length;
                    $("#option_div_list > *:not('#option_div_temp')").remove();
