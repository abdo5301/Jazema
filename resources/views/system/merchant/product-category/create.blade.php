                                        ...
                                        <div class="card-block card-dashboard">
                                            <div class="form-group col-sm-12 {!! formError($errors,'merchant_category_id',true) !!}">
                                                <div class="controls">
                                                    ...
                                                    @if(isset($MerchantCategoryData))
                                                        {!! Form::text('merchant_text', $MerchantCategoryData->{'name_'.\DataLanguage::get()}.' #ID: '.$MerchantCategoryData->id,['class'=>'form-control','readonly'=>'readonly']) !!}
                                                        {!! Form::hidden('merchant_category_id',null,['id'=>'merchant_category_id']) !!}
                                                    @else

                                                        @if(isset($result->id))
                                                            {!! Form::select('merchant_category_id',[$result->merchant_category->id => $result->merchant_category->{'name_'.\DataLanguage::get()}.' #ID: '.$result->merchant_category_id],isset($result->id) ? $result->merchant_category_id:old('merchant_category_id'),['class'=>'select2 form-control']) !!}
                                                        @else
                                                            {!! Form::select('merchant_category_id',[__('Select Merchant Category')],old('merchant_category_id'),['style'=>'width: 100%;','class'=>'select2 form-control']) !!}
                                                        @endif
                                                    @endif
...
@section('footer')
    ...
    <script>
        ...
        $(function(){
            ajaxSelect2('#merchant_category_id','merchant_category');
