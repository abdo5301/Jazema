        ...
        <div class="content-wrapper">
            <div class="content-header row">
                <div class="content-header-left col-md-4 col-xs-12">
                    <h4>
                        {{$pageTitle}}
                        @if(\Request::route()->getName() == 'merchant.merchant.create')
                            <a class="btn btn-danger" style="color: #fff;" href="javascript:;" onclick="return disapproved();">{{__('Disapproved')}}</a>
                        @endif
                    </h4>
                </div>
                                        ...
                                        <div class="col-sm-12">
                                            <div class="card">
                                                <div class="card-block card-dashboard">
                                                    ...
                                                    <div class="form-group col-sm-12{!! formError($errors,'parent_id',true) !!}">
                                                        <div class="controls">
                                                            ...
                                                            @if(isset($result->parent_id))
                                                                {!! Form::select('parent_id',[$result->parent->id => $result->parent->{'name_'.\DataLanguage::get()}.' #ID: '.$result->parent_id],isset($result->id) ? $result->parent_id:old('parent_id'),['class'=>'select2 form-control']) !!}
                                                            @else
                                                    ...
                                                    @if(isset($tempData) && array_key_exists($tempData->create_id,$realSalesMen))
                                                        <div class="clearfix"></div>
                                                        <div class="form-group col-sm-12">
                                                            <div class="controls">
                                                                <b>{{$realSalesMen[$tempData->create_id]}}</b>
                                                            </div>
                                                        </div>

                                                        {!! Form::hidden('staff_id',$tempData->create_id) !!}
                                                                ...
                                                                @foreach($tempData['uploads'] as $oneupload)
                                                                    <div class="onfile col-sm-12 div-with-files">
                                                                        <div class="col-sm-5 form-group">
                                                                            ...
                                                                            <input type="text" class="form-control" value="{{$oneupload->title}}" >
                                                                        </div>
                                                                        <div class="col-sm-5 form-group">
                                        ...
                                        <div class="col-sm-12">
                                            <div class="card">
                                                <div class="card-block card-dashboard">
                                                    ...
                                                    <div class="form-group col-sm-4{!! formError($errors,'staff_national_id',true) !!}">
                                                        <div class="controls">
                                                    ...
                                                    <div class="form-group col-sm-4{!! formError($errors,'staff_email',true) !!}">
                                                        <div class="controls">
                                                    ...
                                                    <div class="form-group col-sm-4{!! formError($errors,'staff_mobile',true) !!}">
                                                        <div class="controls">
                                                    ...
                                                    @if(isset($tempData))
                                                        <div class="form-group col-sm-12{!! formError($errors,'staff_birthdate',true) !!}">
                                                            @else
                                ...
                                <div class="col-sm-12">
                                    <div class="card">
                                        <div class="card-block card-dashboard">
                                            ...
                                            <div class="form-group col-sm-12{!! formError($errors,'is_reseller',true) !!}">
                                                <div class="controls">
                                                    {!! Form::label('is_reseller', __('is Reseller').':') !!}
                                                    {!! Form::select('is_reseller',['in-active'=>__('No'),'active'=>__('Yes')],isset($result->id) ? $result->is_reseller:old('is_reseller'),['class'=>'form-control']) !!}
                                                </div>
                                            ...
                                            <div id="parent_id_div" class="form-group col-sm-12{!! formError($errors,'parent_id',true) !!}">
                                                <div class="controls">
                                                    {!! Form::label('parent_id', __('Parent From Merchant').':') !!}
                                                    @if(isset($result->parent_id))
                                                        <a href="javascript:void(0);" onclick="if(confirm('{{__('are you sure you want to remove reseller?')}}')) $('#parent_id').html('<option value=\'\'>{{__('Select Reseller')}}</option>');">{{__('Remove Reseller')}}</a>
                                                        {!! Form::select('parent_id',[$result->parent->id => $result->parent->{'name_'.\DataLanguage::get()}.' #ID: '.$result->parent_id],isset($result->id) ? $result->parent_id:old('parent_id'),['class'=>'select2 form-control','id'=>'parent_id']) !!}
                                                    @else
                                                        {!! Form::select('parent_id',[__('Select Reseller')],old('parent_id'),['style'=>'width: 100%;','class'=>'select2 form-control']) !!}
                                                    @endif
                                                </div>
                                                {!! formError($errors,'parent_id') !!}
                                            </div>
                                            ...
                                            <div id="reseller_commission_type_div" class="form-group col-sm-12{!! formError($errors,'include_merchant_under_agent_commission',true) !!}">
                                                <div class="controls">
                                                    {!! Form::label('include_merchant_under_agent_commission', __('Include This Merchant Under Agent Commission').':') !!}
                                                    {!! Form::select('include_merchant_under_agent_commission',['no'=>__('No'),'yes'=>__('Yes')],isset($result->id) ? $result->include_merchant_under_agent_commission:old('include_merchant_under_agent_commission'),['class'=>'form-control']) !!}
                                                </div>
                                                {!! formError($errors,'include_merchant_under_agent_commission') !!}
                                            </div>
                                            ...
                                            @if($sales_data)
                                                <div class="form-group col-sm-4{!! formError($errors,'staff_id',true) !!}">
                                                    <div class="controls">
                                                        {!! Form::label('staff_id', __('Sales Man').':') !!}
                                                        @if(isset($result->staff_id))
                                                            {!! Form::select('staff_id',$sales_data,isset($result->id) ? $result->staff_id:old('staff_id'),['class'=>'select2 form-control']) !!}
                                                        @else
                                                            {!! Form::select('staff_id',$sales_data,old('staff_id'),['class'=>'form-control','readeonly'=>true]) !!}
                                                        @endif
                                                    </div>
                                            ...
                                            <div class="form-group col-sm-12{!! formError($errors,'special_commission_list_id',true) !!}">
                                                <div class="controls">
                                                    {!! Form::label('special_commission_list_id', __('Commission List').':') !!}
                                                    {!! Form::select('special_commission_list_id',$special_commission_list,isset($result->id) ? $result->special_commission_list_id:old('special_commission_list_id'),['class'=>'form-control']) !!}
                                                </div>
                                                {!! formError($errors,'special_commission_list_id') !!}
                                            </div>
    ...
    <script>
        ...
        $(function(){
            ...
            if($('#is_reseller').val()=='active'){
                $('#parent_id_div').hide();
                $('#reseller_commission_type_div').show();
            } else {
                $('#parent_id_div').show();
                $('#reseller_commission_type_div').hide();
            }
        ...
        function disapproved(){
            $comment = prompt('{{__('Reason ?')}}');

            if(!empty($comment)){
                window.location = '{{route('merchant.merchant.review',['disapproved'=>request('tempData')])}}&disapproved_comment='+$comment;
                return true;
            }
